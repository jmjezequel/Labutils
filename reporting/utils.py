...
def ratio(v1,v2):
    return "0%" if v2==0 else str(round(v1*100/v2))+'%'
...
class Report:
    ...
    def ratioOfDiff(self, struct: SubStructure,additionalCondition,baseCondition):
        return ratio(self.tmembers(struct,lambda m: baseCondition(m) and additionalCondition(m)),self.tmembers(struct,baseCondition))
    ...
    def tdocpubs(self, struct: SubStructure,cond):
         ...
         return self.lab.getTotalOf(self.startDate, self.endDate, lambda m: len(m.getPhDRawPubList()) - 1, struct, cond)  # -1 to exclude own PhD document
    ...
    def mduration(self, struct,cond):
        return math.floor(self.lab.getMeanOf(self.startDate, self.endDate, lambda m: m.getThesisDuration(), struct, cond))
